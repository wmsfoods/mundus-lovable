...
function computeFinalPrice(
  ...
  // primary FOB/EXW, chosen needs freight/insurance: ADD
  if ((primaryEff === "FOB" || primaryEff === "EXW") && (ch === "CFR" || ch === "CNF" || ch === "CIF")) {
    let final = basePrice + freightPerKg;
    const parts: Breakdown["parts"] = { freight: "add" };
    if (ch === "CIF") {
  ...
  // primary includes freight, chosen FOB/EXW: SUBTRACT freight
  if ((primaryEff === "CFR" || primaryEff === "CNF" || primaryEff === "CIF") && (ch === "FOB" || ch === "EXW")) {
    let final = Math.max(0, basePrice - freightPerKg);
    const parts: Breakdown["parts"] = { freight: "subtract" };
    if (primaryEff === "CIF") {
      // CIF had insurance baked too — remove
  ...
  // primary CFR -> CIF : add insurance only
  ...
  // primary CIF -> CFR : subtract insurance only
