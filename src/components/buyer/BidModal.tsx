...
export function BidModal({ open, onOpenChange, offer }: BidModalProps) {
    ...
    setBidDrafts(
      Object.fromEntries(
        Object.entries(next).map(([id, kg]) => [
          id,
          toDisplay(kg, "price", unit).toFixed(negoPriceDigits(unit)),
        ]),
      ),
    );
  };
    ...
    for (const it of offer.items) {
      ...
      if (v == null || !Number.isFinite(v) || v <= 0) {
        ...
        out[it.id] = t("buyer.bid.validation.minPct", {
          ...
          min: toDisplay(min, "price", unit).toFixed(negoPriceDigits(unit)),
        });
      }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // AUTO-NEGOTIATION: if the offer is opted into auto mode, immediately
      // invoke the propose-counter edge function so the buyer sees a counter
      // generated by the engine. Manual offers (the default) skip this.
      try {
        const { data: offerRow } = await (supabase as any)
          .from("offers")
          .select("negotiation_mode")
          .eq("id", offer.id)
          .maybeSingle();
      ...
      <DialogContent className="max-w-[720px] max-h-[90vh] overflow-y-auto sm:rounded-lg max-sm:!max-w-full max-sm:!max-h-[100dvh] max-sm:!h-[100dvh] max-sm:!rounded-none max-sm:!m-0">
          ...
          <label className="flex flex-col gap-1 text-sm">
            ...
            {selectedFreight && (
              <span className="text-xs text-muted-foreground">
                ...
                {fmtPriceNego(freightPerKg, unit)} {pLbl})
                {insurancePerKg > 0 && (
                  <> · Insurance {fmtPriceNego(insurancePerKg, unit)} {pLbl}</>
                )}
              </span>
            )}
          </label>
                      ...
                      <div className="flex flex-col items-end gap-0.5">
                        <Input
                          ...
                          onBlur={(e) => {
                            ...
                            setBidDrafts((prev) => ({ ...prev, [it.id]: v.toFixed(negoPriceDigits(unit)) }));
                            setBids((prev) => ({ ...prev, [it.id]: fromDisplay(v, "price", unit) }));
                          }}
                          onKeyDown={(e) => {
                        ...
                        {!showErr && (
                          <span className="text-[10px] text-muted-foreground">
                            Min: ${toDisplay(asking * MIN_BID_PCT, "price", unit).toFixed(negoPriceDigits(unit))}/{wLbl} (70% of asking)
                          </span>
                        )}
                        {Math.abs(d) > 0.001 && (
                          <span
                          ...
                          >
                            {d < 0 ? "↓" : "↑"} ${Math.abs(displayDiff).toFixed(negoPriceDigits(unit))} ({d < 0 ? "" : "+"}
                            {dPct.toFixed(1)}%)
                          </span>
                        )}
                      </div>
        ...
        {selectedFreight && (freightPerKg > 0 || insurancePerKg > 0) && (
          <div className="text-xs text-muted-foreground mt-1">
            Freight US$ {Number(selectedFreight.cost).toLocaleString()} ({fmtPriceNego(freightPerKg, unit)} {pLbl})
            {insurancePerKg > 0 && selectedFreight.insurance != null && (
              <> · Insurance US$ {Number(selectedFreight.insurance).toLocaleString()} ({fmtPriceNego(insurancePerKg, unit)} {pLbl})</>
            )}
