-- Allow 'sold_out' status on offers
...
ALTER TABLE public.offers ADD CONSTRAINT offers_status_check
  CHECK (status = ANY (ARRAY['draft','active','inactive','archived','sold_out']));

-- Add negotiation_id on orders + backfill
ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS negotiation_id uuid REFERENCES public.negotiations(id);
CREATE INDEX IF NOT EXISTS idx_orders_negotiation_id ON public.orders(negotiation_id);

UPDATE public.orders o SET negotiation_id = n.id
  FROM public.negotiations n
 WHERE n.order_id = o.id AND o.negotiation_id IS NULL;

-- Update accept_negotiation to set negotiation_id + auto sold_out
...
BEGIN
  ...
  ELSE
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      'offer_item_id', cr.offer_item_id, 'price_per_kg', cp.price_per_kg
    )), '[]'::jsonb) INTO v_agreed
    FROM cut_rounds cr JOIN counter_proposals cp ON cp.cut_round_id = cr.id
    WHERE cr.round_proposal_id = v_last_round_id;
  END IF;
...
-- Backfill: mark already fully-sold offers as sold_out
WITH sold AS (
  SELECT offer_id, SUM(fcl_count) as total_sold
    FROM public.negotiations
   WHERE status = 'bid_accepted' AND deleted_at IS NULL
   GROUP BY offer_id
)
UPDATE public.offers o SET status='sold_out'
  FROM sold s
 WHERE s.offer_id = o.id
   AND s.total_sold >= COALESCE(o.total_fcl, 1)
