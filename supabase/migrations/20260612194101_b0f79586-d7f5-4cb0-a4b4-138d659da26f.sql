...
-- 1) Rewrite link function so company_users insert errors are not swallowed silently,
--    and so the master role is always applied for the first user of a company.
...
BEGIN
  ...
  -- NO LONGER swallow errors: we WANT this to surface so the first user is always
  -- registered as Master and shows up on the Team tab.
...
-- 2) Backfill: for every approved user_request whose auth user exists, make sure
--    there is a Master row in company_users for the approved company.
DO $$
DECLARE
  r record;
  v_uid uuid;
...
BEGIN
  FOR r IN
    SELECT ur.id, ur.email, ur.name, ur.approved_company_id
      FROM public.user_requests ur
     WHERE ur.status = 'approved'
       AND ur.approved_company_id IS NOT NULL
  LOOP
    ...
    IF EXISTS (SELECT 1 FROM public.company_users cu
                WHERE cu.company_id = r.approved_company_id AND cu.user_id = v_uid) THEN
      CONTINUE;
    END IF;

    v_is_supplier := EXISTS (SELECT 1 FROM public.companies c
                              WHERE c.id = r.approved_company_id AND COALESCE(c.is_supplier,false) = true);
    v_role_text := CASE WHEN v_is_supplier THEN 'master_supplier' ELSE 'master_buyer' END;
    SELECT id INTO v_role_id FROM public.roles WHERE name = v_role_text LIMIT 1;
    v_full_name := COALESCE(NULLIF(r.name,''), split_part(r.email,'@',1));

    BEGIN
      INSERT INTO public.company_users
        (company_id, user_id, email, full_name, role, role_id, status, accepted_at, joined_at)
      VALUES
        (r.approved_company_id, v_uid, lower(r.email), v_full_name, v_role_text, v_role_id, 'active', now(), now())
      ON CONFLICT (company_id, user_id) DO NOTHING;
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'backfill skipped for %: %', r.email, SQLERRM;
    END;
  END LOOP;
END $$;
