...
BEGIN
  ...
  -- Allow both supplier (awaiting_supplier) and buyer (pending_buyer_review) to accept.
  ...
  ELSIF v_last_round_id IS NOT NULL THEN
    -- Prefer engine counter price; fall back to manual cut_rounds price.
  ...
  ELSE
    -- Build agreed_items from last round, preferring counter_proposals when present.
