...
BEGIN
  ...
  IF v_standard_product_id IS NULL THEN
    v_product_number := v_cut.product_number;
    IF v_product_number IS NULL
       OR EXISTS (
         SELECT 1 FROM public.standard_products
    ...
    VALUES (v_category_id, v_cut.name, true, v_product_number)
