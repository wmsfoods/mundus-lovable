...
interface RequestBody { negotiation_id: string; reason?: string; }
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  if (typeof body.negotiation_id !== 'string') {
    return errorResponse('invalid_payload', 'Expected { negotiation_id: uuid, reason?: string }', 400);
  }
  if (body.reason !== undefined && typeof body.reason !== 'string') {
    return errorResponse('invalid_payload', 'reason must be a string', 400);
  }
  ...
  const { data: rpcResult, error: rpcErr } = await admin.rpc('reject_negotiation', {
